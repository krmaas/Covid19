...
```{r nyTimesCounty}
nytcounty <- read.csv("https://raw.githubusercontent.com/nytimes/covid-19-data/master/us-counties.csv", header = T)
...
# trying to figure out what I'm joining my data to, counties is from urbanmaps
# counties <- counties
...
```{r caseFatalityRateUndertesting}
p+    geom_polygon(aes(fill=deaths/cases))+
  geom_polygon(data = states, mapping = aes(long, lat, group = group), fill = NA, color = "light grey") +
...
anim <- nytcountypop %>%
  ...
  geom_polygon(data = states, mapping = aes(long, lat, group = group), fill = NA, color = "light grey") +
  scale_fill_viridis_c(option = "viridis",direction = -1, limits = c(0, 10))+
  coord_map(projection = "albers", lat0 = 39, lat1=45)+
  labs(fill="Cases per capita")+
  theme_bw()+
  theme(legend.position = "bottom")+
  transition_time(time=date)+
    ggtitle('Now showing {frame_time}')
animate(anim, renderer = gifski_renderer(), end_pause = 10, fps = 5)
