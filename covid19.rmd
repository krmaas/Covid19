...
ggplot() + 
  geom_boxplot(aes(x=as.Date(date), y=cases, group = interaction(date, caseType), fill=factor(caseType)), color = "light grey")+
  # facet_geo(~state, grid=my_gird, scales="free_y")+ #wraping by state location
    # scale_y_continuous(labels = scales::comma)+
  scale_y_log10()+
  # scale_x_continuous(limits = c())+
  scale_fill_brewer(palette = "Spectral", drop=TRUE)+
  theme_classic()+
  theme(axis.text.x = element_text(angle = 60, hjust = 1))+
