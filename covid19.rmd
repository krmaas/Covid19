...
## get population data from CRAN packages
...
ggplot(todayscovid19, aes(x=confirmedCases*100000/pop, y=deaths*100000/pop, color=continent))+
    geom_point(size=3)+
    theme_bw()+
    scale_color_brewer(palette="Set1")+
    geom_text(data=subset(todayscovid19, confirmedCases*100000/pop > 150), 
              aes(x=confirmedCases*100000/pop, y=deaths*100000/pop,label=Country.Region), vjust="inward", hjust="inward", color="Black")+
    labs(x="Cases per 100,000", y="Deaths per 100,000")+
    geom_text(data=subset(todayscovid19, Country.Region == "United States"), 
               aes(x=confirmedCases*100000/pop, y=deaths*100000/pop, label="US"), size=5, color="black")
...
anim <- ggplot(covid19, aes(x=confirmedCases*100000/pop, y=deaths*100000/pop, color=continent))+
    geom_point(size=3)+
    theme_bw()+
    scale_color_brewer(palette="Set1")+
    # geom_text(data=subset(covid19, confirmedCases*100000/pop > 150),
              # aes(x=confirmedCases*100000/pop, y=deaths*100000/pop,label=Country.Region), vjust="inward", hjust="inward", color="Black")+
    labs(x="Cases per 100,000", y="Deaths per 100,000")+
...
animate(anim, renderer = gifski_renderer(), end_pause = 10)
anim_save(filename="casesDeathsPerCapitaText.gif", animation=last_animation())
...
usAbb <- read.csv("https://raw.githubusercontent.com/jasonong/List-of-US-States/master/states.csv", header=T)
...
usDailyAll <- left_join(usDaily, usPop, by=c("state" = "Abbreviation")) %>%
    mutate(posTotalPerCap = (positive*100000)/POPESTIMATE2019,
           negTotalPerCap = (negative*100000)/POPESTIMATE2019,
           hospTotalPerCap = (hospitalized*100000)/POPESTIMATE2019,
           deathTotalPerCap = (death*100000)/POPESTIMATE2019,
           totalTotalPerCap = (total*100000)/POPESTIMATE2019,
        deathPerCap = (deathIncrease*100000)/POPESTIMATE2019,
           hospitalizedPerCap = (hospitalizedIncrease*100000)/POPESTIMATE2019,
           positivePerCap = (positiveIncrease*100000)/POPESTIMATE2019,
            testsPerCap = (totalTestResultsIncrease*100000)/POPESTIMATE2019)
...
```{r covidProjectPosState, warning = FALSE, fig.height=15, fig.width=10}
...
ggplot(usDailyAll, aes(x=date, y=posTotalPerCap))+
  ...
  theme_bw()+
  theme(axis.text.x = element_text(angle = 60, hjust = 1))+
  ggtitle("Positive test per 100,000 by state")
```
...
```{r covidProjectTestingState, warning = FALSE, fig.height=15, fig.width=10}
...
ggplot(usDailyAll, aes(x=date, y=totalTotalPerCap))+
  ...
  theme_bw()+
  theme(axis.text.x = element_text(angle = 60, hjust = 1))+
  ggtitle("Total test per 100,000 by state")

```

#### Plotting the increases

```{r increases, warning = FALSE}
...
```{r increasesPop, warning = FALSE, fig.height=15, fig.width=10}
...
usIncPop <- left_join(usDailyIncrease, usPop, by=c("state" = "Abbreviation")) %>%
    mutate(deathPerCap = (deathIncrease*100000)/POPESTIMATE2019,
           hospitalizedPerCap = (hospitalizedIncrease*100000)/POPESTIMATE2019,
           positivePerCap = (positiveIncrease*100000)/POPESTIMATE2019,
            testsPerCap = (totalTestResultsIncrease*100000)/POPESTIMATE2019)
...
ggplot(usIncPop, aes(x=date, y=testsPerCap))+
  ...
  theme_bw()+
  theme(axis.text.x = element_text(angle = 60, hjust = 1))+
  ggtitle("Tests per 100,000 by state")
